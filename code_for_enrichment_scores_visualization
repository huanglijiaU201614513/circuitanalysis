...
enrichment_single <- function(all.circuits, all.scores, motif_list, new_ind, decreasing = T, topCircuits  = 600, nhill = 20) {
  ...
  ntot_circuit = length(all.scores)
  ...
  if(decreasing){
    all.order = order(all.scores, decreasing = decreasing)
    all.scores_sorted = all.scores[all.order]
    score_threshold = all.scores_sorted[topCircuits]
    
    for (i in 1:ntot_circuit){
      circuit = all.circuits[[all.order[i]]]
    ...
    all.order = order(all.scores, decreasing = increasing)
    all.scores_sorted = all.scores[all.order]
    score_threshold = all.scores_sorted[topCircuits]
...
library("ggplot2")
library("pheatmap")
load("/Users/lijiahuang/Desktop/projects/ALLUNIQUEMODELS_final.Rdata")
csv1<-read.csv("/Users/lijiahuang/Desktop/projects/alltestresult61.csv")
all.scores = csv1$HH
enrichment_random <- function(all.circuits, all.scores, motif_list, new_ind, decreasing = T, topCircuits  = 600, nhill = 20) {
  original_order = c(1:60212)
  ntot_circuit = length(all.scores)
  ntot_motif = 39  # a total of 39 non-redundant 2-node motifs
  ...
  all.order = sample(original_order)
  all.scores_sorted = all.scores[all.order]
  score_threshold = all.scores_sorted[topCircuits]
...
outcome_random = enrichment_random(ALLUNIQUEMODELS, all.scores, motif_list, new_ind, decreasing = T, topCircuit = 600) 
...
### the following data need to be generated first. They speedup the calculation
### these data are required as the arguments for the motif searching functions
motif_list = generate_motif_list()
new_ind = generate_index_conversion()
overlap_list = generate_overlap_data()

### test circuit
circuit_test <- matrix(c(2,2,2,0,2,2,0,2,2,0,0,0,0,2,0,2), ncol = 4)
dimnames(circuit_test) = list(LETTERS[1:4], LETTERS[1:4])
plot.adj(circuit_test)

### find all 2-node motifs
find_motif_2node(circuit_test, motif_list, new_ind)
plot.motif(24, motif_list)
plot.motif(22, motif_list)

### find all pairwise motif coupling, I didn't output combination indices, as they are not needed
#find_interactions_2node(circuit_test, motif_list, new_ind)
find_interactions_2node_new(circuit_test, motif_list, new_ind, overlap_list)

### 2nd test 
circuit_test <- matrix(c(1,0,0,2,2,1,0,0,2,1,1,0,0,2,2,1), ncol = 4)
dimnames(circuit_test) = list(LETTERS[1:4], LETTERS[1:4])
plot.adj(circuit_test)
find_motif_2node(circuit_test, motif_list, new_ind)
plot.motif(12, motif_list)
plot.motif(3, motif_list)
find_interactions_2node_new(circuit_test, motif_list, new_ind, overlap_list)
...
#svg("/Users/lijiahuang/Desktop/projects/project\ 1/figure_5a.svg", width=12, height=6)     
ggplot(outcome_single, aes(x= reorder(row.names(outcome_single), -lfc), lfc)) + geom_bar(stat="identity") + theme_bw() + theme(panel.grid=element_blank())+ theme(plot.margin=unit(rep(1,4),'cm'))
#dev.off()
outcome_coupling = enrichment_coupling(ALLUNIQUEMODELS, all.scores, motif_list, new_ind, overlap_list, 
                                       if_overlap = 2, decreasing = T, topCircuit = 600) 
...
pheatmap(outcome_coupling_all$t0,fontsize = 8)
pheatmap(outcome_coupling_all$t1,fontsize = 8)
pheatmap(outcome_coupling_all$t2,fontsize = 8)
